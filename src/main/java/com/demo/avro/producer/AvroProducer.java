...
public class AvroProducer {
	...
	public static void main(String[] args) {
		...
		try {
			for (long i = 0; i < 2; i++) {
				Order order = new Order("order_id" + i, "customer_id", "supplier_id", "cust_name", 10, (float) 100);
				ProducerRecord<String, Order> record = new ProducerRecord<String, Order>(topic,
						(String) order.getOrderId(), order);
