...
public class AvroReadFromKafkaWriteToDB {
	...
	private static final String ORDER_INSERT_QUERY = "insert into ORDER_DETAILS  values(?, ?,?,?,?,?,?,?)";
	...
	public static void main(String[] args) {
		...
		PCollection<Data> output = p
				.apply(KafkaIO.<String, byte[]>read().withBootstrapServers(BOOT_STRAP_SERVER).withTopic(TOPIC_NAME)
				...
				.apply(MapElements.via(new SimpleFunction<KV<String, byte[]>, Data>() {
					...
					public Data apply(KV<String, byte[]> word) {
						...
						BinaryDecoder binaryDecoder = DecoderFactory.get().binaryDecoder(word.getValue(), null);
						try {
		...
		output.apply(
				JdbcIO.<Data>write()
						...
						.withPreparedStatementSetter(new JdbcIO.PreparedStatementSetter<Data>() {
							private static final long serialVersionUID = 1L;
